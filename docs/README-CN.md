...
* **模块化架构**：API 通过 KSP 自动生成注册表完成自动注册，并按包路径自动生成路由（例如 `com.actl.mvp.api.v1.control.click -> /v1/control/click`）。
...
```text
.
├── api-registry-ksp/   # KSP 处理器：生成 API 注册表
├── app/src/main/java/com/actl/mvp/
│   ├── api/
│   │   ├── framework/  # ApiDefinition、服务装配、KSP 注册桥接、路径解析
│   │   └── v1/         # 按路由路径组织的版本化 API 实现
│   ├── adb/
│   │   ├── core/       # 纯 Kotlin ADB 协议、配对与传输实现
│   │   ├── session/    # 共享 DirectAdbManager 运行时与命令结果模型
│   │   └── discovery/  # mDNS 发现与端点状态模型
│   ├── startup/
│   │   └── service/    # 配对与 API 服务生命周期前台服务
│   └── ime/            # 自定义输入法服务
├── clients/            # 外部工具目录（不属于核心 App 运行时）
│   └── llm-controller/ # 调用 DroidNode API 的控制器 Demo
├── tests/              # API/性能/构建测试脚本 (Python/Shell)
├── tools/              # 本地工具脚本（agent/资源处理），不属于核心运行时
└── LICENSE             # 开源许可证
